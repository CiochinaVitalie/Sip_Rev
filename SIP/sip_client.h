...
extern uint32_t _Min_Heap_Size; // Minimum heap size defined in linker script
...
//                audioTX(socket);
                ...
                void* heap_end = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(&_estack) - reinterpret_cast<uintptr_t>(&_Min_Heap_Size));
                statO.freeRam = (size_t)heap_end - (size_t)sbrk(0);
...
private:
    ...
    void send_sip_invite()
    {
        ...
        tx_buffer << "Content-Length: " << static_cast<uint32_t>(m_tx_sdp_buffer.size()) << "\r\n";
        tx_buffer << "\r\n";
